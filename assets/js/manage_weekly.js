$(document).ready(function () {
    ...
    if (typeof manage_weekly_data === "undefined") {
        console.error(
            "[Weekly JS Debug] ERRO: manage_weekly_data não está definido!",
        );
        alert(
            "ERRO: Dados da página não foram carregados corretamente. Verifique os logs do servidor.",
        );
        return;
    }
    ...
    if (!manage_weekly_data.weekly_approvals) {
        console.error(
            "[Weekly JS Debug] ERRO: weekly_approvals não existe nos dados!",
        );
        console.log(
            "[Weekly JS Debug] Propriedades disponíveis:",
            Object.keys(manage_weekly_data),
        );
        return;
    }

    console.log(
        "[Weekly JS Debug] Aprovações semanais encontradas:",
        manage_weekly_data.weekly_approvals.length,
    );
    ...
    if (manage_weekly_data.weekly_approvals.length > 0) {
        manage_weekly_data.weekly_approvals.forEach(function (approval, index) {
            console.log(
                "[Weekly JS Debug] Aprovação " + (index + 1) + ":",
                approval,
            );
            console.log("[Weekly JS Debug] - Staff ID:", approval.staff_id);
            console.log(
                "[Weekly JS Debug] - Nome:",
                approval.firstname + " " + approval.lastname,
            );
            console.log("[Weekly JS Debug] - Status:", approval.status);
            console.log(
                "[Weekly JS Debug] - Total tarefas:",
                approval.total_tasks,
            );

            loadTotalHours(
                approval.id,
                approval.staff_id,
                approval.week_start_date,
            );
            loadTimesheetPreview(
                approval.id,
                approval.staff_id,
                approval.week_start_date,
            );
        ...
        console.log(
            "[Weekly JS Debug] Nenhuma aprovação encontrada para esta semana",
        );
    }
    ...
    function loadTotalHours(approvalId, staffId, weekStartDate) {
        ...
        if ($totalDisplay.length === 0) {
            console.warn(
                "[Weekly JS Debug] Elemento total-hours-display não encontrado para approval:",
                approvalId,
            );
            return;
        }
        ...
        $.get(manage_weekly_data.admin_url + "timesheet/get_week_total", {
            ...
            .done(function (response) {
                console.log(
                    "[Weekly JS Debug] Resposta total de horas:",
                    response,
                );
                if (response.success) {
                    $totalDisplay.html(
                        "<strong>" +
                            parseFloat(response.total_hours).toFixed(1) +
                            "h</strong>",
                    );
                } else {
            ...
            .fail(function (xhr, status, error) {
                console.error(
                    "[Weekly JS Debug] Erro ao carregar total de horas:",
                    error,
                );
                $totalDisplay.html('<span class="text-danger">Erro</span>');
            });
    ...
    // Function to load timesheet preview - VERSÃO SIMPLIFICADA
    function loadTimesheetPreview(approvalId, staffId, weekStartDate) {
        ...
        if ($previewContainer.length === 0) {
            console.warn(
                "[Weekly JS Debug] Container de preview não encontrado para approval:",
                approvalId,
            );
            return;
        }
        ...
        $.get(
            manage_weekly_data.admin_url + "timesheet/get_timesheet_preview",
            {
                staff_id: staffId,
                week_start_date: weekStartDate,
            },
            ...
            .done(function (response) {
                ...
                if (response.success) {
                    ...
                    $previewContainer.html(
                        '<div class="text-center text-danger">Erro ao carregar preview</div>',
                    );
                }
            })
            .fail(function (xhr, status, error) {
                console.error(
                    "[Weekly JS Debug] Erro ao carregar preview:",
                    error,
                );
                $previewContainer.html(
                    '<div class="text-center text-danger">Erro ao carregar dados</div>',
                );
            });
    }
    ...
    $(document).on("click", "button.approve-btn, a.approve-btn", function (e) {
        ...
        console.log(
            "[Weekly JS Debug] Clicou em aprovar semanal, ID:",
            approvalId,
        );
        ...
        TimesheetModals.confirm({
            ...
            message:
                "Tem certeza que deseja aprovar este timesheet? Esta ação não pode ser desfeita.",
            ...
            console.log(
                "[Weekly JS Debug] Resultado da confirmação semanal:",
                confirmed,
            );
            if (confirmed) {
    ...
    $(document).on("click", ".cancel-approval-btn", function (e) {
        ...
        if (typeof TimesheetModals !== "undefined") {
            TimesheetModals.confirm({
                ...
                message:
                    "Tem certeza que deseja cancelar esta aprovação? O timesheet voltará ao status de rascunho e os timers do quadro de horas serão removidos.",
                ...
                console.log(
                    "Resultado da confirmação de cancelamento:",
                    confirmed,
                );
                if (confirmed) {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    ...
                    $(
                        '.total-hours-display[data-approval-id="' +
                            approvalId +
                            '"]',
                    ).html(formatted);
                } else {
    ...
    function loadTimesheetPreview(approvalId, staffId, weekStartDate) {
        $.ajax({
            url:
                manage_weekly_data.admin_url +
                "timesheet/get_timesheet_preview",
            type: "GET",
            data: {
                staff_id: staffId,
                week_start_date: weekStartDate,
            ...
            success: function (response) {
                if (response.success && response.html) {
                    ...
                    $("#preview-" + approvalId).html(
                        '<div class="text-center text-muted">Nenhuma entrada encontrada</div>',
                    );
                }
            },
            error: function () {
                $("#preview-" + approvalId).html(
                    '<div class="text-center text-muted text-danger">Erro ao carregar preview</div>',
                );
            },
        });
    }
            ...
            success: function (response) {
                ...
                if (response.success) {
                    TimesheetModals.alert({
                        ...
                        $(
                            '.approval-panel[data-approval-id="' +
                                approvalId +
                                '"]',
                    ...
                    TimesheetModals.alert({
                        title: "Erro",
                        message:
                            response.message || "Erro ao processar aprovação",
                        icon: "fa-exclamation-triangle",
                        type: "error",
                    });
            ...
            success: function (response) {
                ...
                if (response.success) {
                    ...
                    TimesheetModals.alert({
                        title: "Erro",
                        message:
                            response.message || "Erro ao cancelar aprovação",
                        icon: "fa-exclamation-triangle",
                        type: "error",
                    });
    ...
    function updateBatchControls() {
        ...
        var totalPending = $(
            '.task-checkbox[data-status="pending"]:enabled',
        ).length;

        // Atualizar contador
        $(".selection-counter").html(
            "<strong>" + selectedCount + "</strong> tarefas selecionadas",
        );

        // Habilitar/desabilitar botões de ação
        $(".batch-approve-btn, .batch-reject-btn").prop(
            "disabled",
            selectedCount === 0,
        );
    ...
    $(document).on("click", ".batch-reject-btn", function () {
        ...
        TimesheetModals.prompt({
            ...
            message:
                "Informe o motivo para rejeitar " +
                selectedTasks.length +
                " tarefas selecionadas:",
            placeholder: "Digite o motivo da rejeição...",
            icon: "fa-times-circle",
    ...
    $(document).on("click", ".user-batch-approve-btn", function () {
        ...
        TimesheetModals.confirm({
            ...
            message:
                "Tem certeza que deseja aprovar " +
                userSelectedTasks.length +
                " tarefas selecionadas deste usuário?",
            icon: "fa-check-circle",
    ...
    function processBatchAction(action, reason, specificTasks) {
        ...
        var buttonSelector = specificTasks
            ? ".user-batch-" + action.replace("ed", "") + "-btn"
            : ".batch-" + action.replace("ed", "") + "-btn";
        ...
        if (specificTasks && specificTasks.length > 0) {
            ...
            var firstTaskCheckbox = $(
                '.task-checkbox[value="' + specificTasks[0] + '"]',
            );
        ...
        $.ajax({
            url:
                manage_weekly_data.admin_url + "timesheet/batch_approve_reject",
            type: "POST",
            data: postData,
            dataType: "json",
            success: function (response) {
                ...
                if (response.success) {
                    ...
                    TimesheetModals.alert({
                        ...
                        message:
                            response.message ||
                            "Erro ao processar ação em lote",
                        icon: "fa-exclamation-triangle",
                        type: "error",
                    });
                    ...
                    $button
                        .html(originalText)
                        .prop("disabled", tasksToProcess.length === 0);
                }
            },
            error: function (xhr, status, error) {
    ...
    function loadTaskDetails(approvalId, staffId, weekStartDate) {
        console.log(
            "[Task Details] Carregando detalhes das tarefas para approval:",
            approvalId,
        );
        ...
        setTimeout(function () {
            $.get(
                manage_weekly_data.admin_url +
                    "timesheet/get_week_task_approvals",
                {
                    staff_id: staffId,
                    week_start_date: weekStartDate,
                },
                ...
                .done(function (response) {
                    ...
                    if (response.success && response.tasks) {
                        ...
                        console.warn(
                            "[Task Details] Nenhuma tarefa encontrada ou erro na resposta",
                        );
                    }
                })
                .fail(function (xhr, status, error) {
    ...
    function renderTaskCheckboxes(approvalId, tasks) {
        console.log(
            "[Task Checkboxes] Iniciando renderização para approval:",
            approvalId,
            "com",
            tasks.length,
            "tarefas",
        ...
        if ($table.length === 0) {
            console.warn(
                "[Task Checkboxes] Tabela não encontrada para approval:",
                approvalId,
            );
            return;
        }
        ...
        if ($table.find(".task-checkbox").length > 0) {
            console.log(
                "[Task Checkboxes] Checkboxes já existem, pulando renderização",
            );
            return;
        }

        // Criar mapa de tasks por identificador único
        var tasksMap = {};
        tasks.forEach(function (task) {
            // Use uma chave única baseada em project_id + task_id
            var key = task.project_id + "_" + task.task_id;
            tasksMap[key] = task;
            console.log(
                "[Task Checkboxes] Mapeando task:",
                key,
                "id:",
                task.id,
                "status:",
        ...
        console.log("[Task Checkboxes] Mapa de tasks criado:", tasksMap);
        ...
        if ($headerRow.length > 0) {
            ...
            var userHeaderHtml =
                '<th width="40" class="text-center"><input type="checkbox" class="select-user-tasks-header" data-user-id="' +
                approvalId +
                '" title="Selecionar todas as tarefas deste usuário"></th>';

            if ($firstTh.find("input#select-all-tasks").length > 0) {
                if (
                    $headerRow.find(
                        '.select-user-tasks-header[data-user-id="' +
                            approvalId +
                            '"]',
                    ).length === 0
                ) {
                    $firstTh.after(userHeaderHtml);
                }
            } else {
                if (
                    $firstTh.find(
                        'input.select-user-tasks-header[data-user-id="' +
                            approvalId +
                            '"]',
                    ).length === 0
                ) {
                    $firstTh.replaceWith(userHeaderHtml);
                }
            }
        }
        ...
        console.log(
            "[Task Checkboxes] Encontradas",
            $dataRows.length,
            "linhas de dados",
        );

        // NOVA LÓGICA: Processar cada linha e tentar identificar a task correspondente
        $dataRows.each(function (rowIndex) {
            var $row = $(this);
            var $firstCell = $row.find("td:first");

            // Extrair informações da linha para identificar a task
            var $projectCell = $row.find("td").eq(1); // Segunda coluna tem projeto/tarefa
            var cellText = $projectCell.text().trim();

            console.log(
                "[Task Checkboxes] Processando linha",
                rowIndex,
                "- Texto:",
                cellText,
            );

            // Tentar encontrar a task correspondente
            var matchedTask = null;
            var taskKey = null;

            // Estratégia 1: Procurar por nome do projeto e tarefa no texto
            for (var key in tasksMap) {
                var task = tasksMap[key];
                var projectName = task.project_name || "";
                var taskName = task.task_name || "";

                // Verificar se o texto da célula contém tanto o projeto quanto a tarefa
                if (
                    cellText.includes(projectName) &&
                    (taskName === "" || cellText.includes(taskName))
                ) {
                    matchedTask = task;
                    taskKey = key;
                    console.log(
                        "[Task Checkboxes] MATCH encontrado para linha",
                        rowIndex,
                        "- Task:",
                        task.id,
                        "Key:",
            ...
            // Se não encontrou match, usar estratégia alternativa por posição (fallback)
            if (!matchedTask && rowIndex < tasks.length) {
                matchedTask = tasks[rowIndex];
                console.log(
                    "[Task Checkboxes] Usando fallback por índice para linha",
                    rowIndex,
                    "- Task:",
                    matchedTask.id,
                );
            ...
            // Inserir checkbox se encontrou task correspondente
            if (matchedTask) {
                var isDisabled =
                    matchedTask.status !== "pending" ? "disabled" : "";
                var checkboxHtml =
                    '<td class="text-center">' +
                    '<input type="checkbox" class="task-checkbox" value="' +
                    matchedTask.id +
                    '" ' +
                    'data-status="' +
                    matchedTask.status +
                ...
                if ($row.find("input.task-checkbox").length === 0) {
                    $firstCell.replaceWith(checkboxHtml);
                    console.log(
                        "[Task Checkboxes] Checkbox inserido para task:",
                        matchedTask.id,
                        "status:",
                        matchedTask.status,
                    );
                ...
                // Linha sem task correspondente
                $firstCell.replaceWith('<td class="text-center">-</td>');
                console.log(
                    "[Task Checkboxes] Nenhuma task encontrada para linha",
                    rowIndex,
                );
            }
        });
        ...
        if ($totalRow.length > 0) {
            ...
            if ($totalFirstCell.find("input").length === 0) {
                $totalFirstCell.replaceWith(
